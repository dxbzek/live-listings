...
sticky WhatsApp FAB · full-screen photo lightbox.
...
array, `PF_URLS`, and `GROUPS`). Listing photos are served from the
PropertyFinder CDN; swap them for your own hosted image URLs over time.
Listing `name` values are kept verbatim from the source PropertyFinder titles.
